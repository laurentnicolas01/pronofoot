...
if(isset($_GET['da'])) {
	$id = intval($_GET['da']);
	echo demande_validate($id) ? '<p class="success">Demande acceptée</p>' : '<p class="error">Erreur de traitement</p>';
}
if(isset($_GET['dr'])) {
	$id = intval($_GET['dr']);
	echo demande_decline($id) ? '<p class="success">Demande refusée</p>' : '<p class="error">Erreur de traitement</p>';
}
